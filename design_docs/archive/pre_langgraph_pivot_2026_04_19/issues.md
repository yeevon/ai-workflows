...
- [x] **SD-03** — Five tiers: `opus` / `sonnet` / `haiku` via Claude Code CLI (Max subscription, no API key), `local_coder` via Ollama (Qwen, free), `gemini_flash` via Gemini API (last-resort overflow only — used when both `haiku` and `local_coder` cannot handle the task). Ollama operational wrapping (health checks, VPN drop handling, `haiku` fallback) lands in M4.
...
- [x] **CRIT-06** — Set `max_retries=0` on every underlying SDK client at adapter construction (Anthropic, OpenAI, httpx). `retry_on_rate_limit()` is the single authority. Prevents 3 × 3 = 9 retry double-amplification and double-counted rate-limit pressure. *Anthropic and OpenAI-compat branches: explicit `max_retries=0`. Google branch: relies on google-genai default `stop_after_attempt(1)` (documented in `_build_google()` + regression test). Fully resolved by M1 Task 10: `primitives/retry.py` exports `retry_on_rate_limit()` as the single authority, and `test_retry.py` pins the SDK-agnostic classifier across both `anthropic` and `openai` exception types.*
...
- [x] **CRIT-08** — Retry error taxonomy (revises P-37). Three classes: retryable-transient (429, 529, `APIConnectionError`, `overloaded_error`) → `retry_on_rate_limit`; retryable-semantic (Pydantic validation, parse errors) → `ModelRetry` feeding the error back to the LLM; non-retryable (`invalid_request_error`, auth) → raise immediately. *Resolved by M1 Task 10: `is_retryable_transient()` classifies 429/529/500/502/503 + `APIConnectionError` + `RateLimitError` across both `anthropic` and `openai` SDKs; `retry_on_rate_limit()` owns the transient loop; `ModelRetry` integration is pinned by `test_model_retry_feeds_error_back_and_model_retries`; non-retryable errors (4xx other than the retryable status set, `ConfigurationError`, `SecurityError`, `BudgetExceeded`) bypass the retry branch and surface on first attempt.*
...
- [x] **P-35** — **REVISED by CRIT-03**: budget limits upgraded from deferred to critical. *Resolved by M1 Task 09: `CostTracker` raises `BudgetExceeded` once the `get_total_cost()` aggregate crosses `budget_cap_usd`.*

### Retry / Rate-Limit / Backoff

- [x] **P-36** — `retry_on_rate_limit()` utility function in primitives.
- [x] **P-37** — **REVISED by CRIT-08**: retry error taxonomy expanded beyond 429/529-only. *Resolved with CRIT-08 by M1 Task 10.*
