...
- **MCP server** (`ai_workflows.mcp`) — **built on FastMCP**: `@mcp.tool()` decorators over pydantic-typed functions; FastMCP generates the JSON-RPC schema, handles stdio/HTTP transport, and runs the server. Exposes:
  ...
  - *(Gate-review projection, M11)* — `RunWorkflowOutput.plan` / `ResumeRunOutput.plan` carry the in-flight draft plan at `status="awaiting", awaiting="gate"`, not only on `status="completed"`. Closes the M9 T04 live-smoke finding that an operator at a HumanGate had nothing to review. No new tool; surface shape unchanged.
