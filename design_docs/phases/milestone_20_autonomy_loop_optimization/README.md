...
|---|---|---|---|---|
| 01 | Sub-agent return-value schema (3-line verdict / file / section), enforced via prompt-mandate + orchestrator-side parsing (Claude Code Task tool does not expose SDK `outputFormat`) | SUPPORT + MODIFY (T01 from #12) | Compaction / doc + code | 📝 Candidate |
...
| # | Task | Research-brief verdict | Phase / Kind | Status |
|---|---|---|---|---|
| 08 | Gate-output integrity (parse raw stdout, fail-closed on missing output, paired with T01's orchestrator-side schema parser as first-defence layer) | SUPPORT + EXTEND (T08 from #10) | Safeguards / code | 📝 Candidate |
