...
4. **(G4)** `/clean-tasks` spec format extension enumerates per-slice file/symbol scope when ACs decompose into file-disjoint slices. Gates the parallel-build flag on this format being present; tasks without it run serial as today. Native `isolation: worktree` frontmatter on the parallel-builder sub-agent. Concurrency capped at 3–4 worktrees. Tasks T18 + T19 stretch — explicit "deferred to M22 if scope-bounded" line in M21 close-out. **(G4 satisfied at T17 — format spec + auto-implement gate check land; T18 parallel-Builder dispatch landed; T19 orchestrator close-out pending)**
