...
5. **(G5)** Quarterly audit prompt over each Skill and slash-command lands as a runnable command (`/audit-skills` or similar). Two-prompt pattern documented in `agent_docs/long_running_pattern.md` with reference Builder loop. (satisfied at T26; pattern locked, agent_docs/ created)
