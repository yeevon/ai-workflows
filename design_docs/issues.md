...
- [~] **CRIT-05** — `ClientCapabilities` descriptor on `LLMClient`. Pydantic model with `supports_prompt_caching`, `supports_parallel_tool_calls`, `supports_structured_output`, `max_context`, `supports_thinking`. Components check capabilities, never `isinstance()` provider class. Prevents layering violations. *Type declared in M1 Task 02; per-adapter factory wiring lands in M1 Task 03.*
