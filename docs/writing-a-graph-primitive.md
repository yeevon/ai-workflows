...
- The pattern would pull a new dependency into `pyproject.toml`. New deps require a new KDR in `design_docs/architecture.md §6` (builder-only, on design branch) plus an ADR — not a primitive.
...
The tests under `tests/graph/` on the `design` branch (builder-only, on design branch) are the reference gallery.
...
- `design_docs/architecture.md §3` (four-layer contract) and `§9` (KDR grid) — the full context for every rule above (builder-only, on design branch).
