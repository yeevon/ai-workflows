"""Tests for M21 Task 13 — /triage post-halt diagnosis Skill.
...
- AC6: This test file (self-referential).
- AC7: _common/skills_pattern.md has the live-Skills count line.
- AC8: CHANGELOG.md updated with M21 Task 13 entry under [Unreleased].
...
subprocess (mirrors test_t25_skills_efficiency.py and test_t24_md_discoverability.py
patterns). No imports from ai_workflows.
"""
...
SKILL_DIR = REPO_ROOT / ".claude" / "skills" / "triage"
...
def _run_discoverability(
    ...
    """Invoke md_discoverability.py with the given check against the triage Skill dir."""
    cmd = [
        sys.executable,
...
def test_skill_md_exists() -> None:
    """AC1: .claude/skills/triage/SKILL.md exists and is non-empty."""
...
def test_skill_md_frontmatter_name() -> None:
    """AC1: frontmatter name == 'triage'."""
    body = _read_skill_md()
    fm = _parse_frontmatter(body)
    assert fm.get("name") == "triage", (
        f"skill name must be 'triage', got {fm.get('name')!r}"
    )
...
def test_runbook_md_exists() -> None:
    """AC2: .claude/skills/triage/runbook.md exists and is non-empty."""
...
def test_runbook_md_t24_summary() -> None:
    ...
    assert result.returncode == 0, (
        f"T24 summary check failed on triage/:\n{result.stdout}\n{result.stderr}"
    )
...
def test_runbook_md_t24_section_budget() -> None:
    ...
    assert result.returncode == 0, (
        f"T24 section-budget check failed on triage/:\n{result.stdout}\n{result.stderr}"
    )
...
def test_runbook_md_t24_code_block_len() -> None:
    ...
    assert result.returncode == 0, (
        f"T24 code-block-len check failed on triage/:\n{result.stdout}\n{result.stderr}"
    )
...
def test_t25_skills_efficiency_clean() -> None:
    """AC3: skills_efficiency --check all is clean across .claude/skills/ (includes triage)."""
    ...
    assert result.returncode == 0, (
        f"skills_efficiency --check all failed after triage/ added:\n"
        f"{result.stdout}\n{result.stderr}"
    )


# ---------------------------------------------------------------------------
# AC7 — _common/skills_pattern.md live-Skills count line
# ---------------------------------------------------------------------------
...
def test_skills_pattern_md_live_skills_count_line() -> None:
    ...
    assert "Live Skills:" in content, (
        ...
        "listing all current Skills (added at T13)"
    )


def test_skills_pattern_md_triage_in_count_line() -> None:
    """AC7: the Live Skills count line mentions 'triage'."""
    content = SKILLS_PATTERN_MD.read_text(encoding="utf-8")
    live_line = next(
        (line for line in content.splitlines() if "Live Skills:" in line), None
    )
    assert live_line is not None, "No 'Live Skills:' line found"
    assert "triage" in live_line, (
        f"'Live Skills:' line must mention 'triage'; got: {live_line!r}"
    )
...
def test_changelog_t13_entry() -> None:
    """AC8: CHANGELOG.md has a M21 Task 13 entry under [Unreleased]."""
    content = CHANGELOG.read_text(encoding="utf-8")
    assert "M21 Task 13" in content, (
        "CHANGELOG.md must contain a 'M21 Task 13' entry under [Unreleased]"
    )
