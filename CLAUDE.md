...
- `/auto-implement <task>` — autonomous-mode `/clean-implement` (run inside the Docker sandbox; see `Dockerfile` + `docker-compose.yml`). Extends `/clean-implement` with (a) a **team gate** after the security gate that runs `sr-dev` + `sr-sdet` and conditionally `architect` (alignment required), (b) auto-commit + auto-push to `design_branch`, and (c) hard halt boundaries (no `main`, no `uv publish`, no KDR conflict). KDR additions land on a separate isolated commit per autonomy decision 2. Halts on team disagreement; sub-agent consensus is the autonomy gate.
- `/queue-pick [<milestone-list>]` — one-shot orchestrator that spawns `roadmap-selector` to pick the next task. Three verdicts: `PROCEED` (recommends `/auto-implement <task>`), `NEEDS-CLEAN-TASKS` (recommends `/clean-tasks <milestone>` first, then re-run `/queue-pick`), `HALT-AND-ASK` (surface the question; user arbitrates). Does not chain — for fully-autonomous queue draining, use `/autopilot`.
- `/autopilot [<milestone-list>]` — autonomous queue-drain meta-loop. Loops over (queue-pick → clean-tasks-if-needed → auto-implement) until the queue drains, the supplied scope is exhausted, or a halt fires. Reads `.claude/commands/auto-implement.md` and `.claude/commands/clean-tasks.md` and executes their procedures inline (not via `Skill` — that halts after one return per memory: `feedback_skill_chaining_reuse.md`). Pre-flight requires `AIW_AUTONOMY_SANDBOX=1` (set in `docker-compose.yml`); HARD HALT on the host. Halts the outer loop on any per-task halt; the user owns retry vs skip.
...
- `roadmap-selector` — picks the next task under autonomous mode using a **sequential default rule** (lowest open milestone → lowest open task) with three eligibility filters (specs hardened, trigger fired, dependencies satisfied) and one narrow override: jump to a later-milestone task only when that task fixes a specific bug or issue that would negatively impact the test or implementation of the Phase-2 candidate. Output: one task ID + rationale, OR halt-and-ask. Read-only; writes only to the recommendation file the invoker names.
...
- **Code-task verification is non-inferential.** For code (Python modules, scripts, surfaces) build-clean is necessary but not sufficient. Every code task spec MUST name an explicit smoke test the Auditor runs (an end-to-end LangGraph run, an MCP tool round-trip, a CLI invocation, a stub-LLM eval). Without one, the spec is incomplete and the audit cannot pass. Inferential claims about runtime behaviour from build success alone are HIGH. **Smoke tests that pre-register workflows via fixtures or that bypass the published CLI/MCP dispatch path do NOT count as wire-level proof** — they would have missed the 0.3.0 spec-API dispatch regression. A wire-level smoke must exercise the same code path a downstream consumer hits.
- **Real-install release smoke.** Every release (every `uv publish`) must run `aiw` against an `uv pip install dist/*.whl` install in a fresh venv, not against the source tree. The pytest gate is `tests/release/test_install_smoke.py` (runs by default — no `AIW_E2E=1` opt-in); the pre-publish bash gate is `scripts/release_smoke.sh` Stage 7. A test that compiles + dispatches through the in-repo source code does not prove the published wheel works for a downstream consumer (0.3.0 shipped a non-functional headline feature because `tests/integration/test_spec_api_e2e.py`'s fixture re-registered the workflow imperatively, side-stepping the broken dispatch lookup). The 0.3.1 hotfix added the install-smoke gate; do not regress it back behind `AIW_E2E`.
...
- **Autonomous-mode boundary (locked 2026-04-27).** Under `/auto-implement`, only the orchestrator runs `git commit` + `git push`, and only against `design_branch`. **No subagent — including `builder`, `auditor`, `security-reviewer`, `dependency-auditor`, `task-analyzer`, `architect`, `sr-dev`, `sr-sdet` — runs `git commit`, `git push`, `git merge`, `git rebase`, `git tag`, or `uv publish`.** A subagent report claiming to have done so is a HARD HALT (rogue behaviour pending user investigation). The orchestrator HARD HALTs on any merge to `main`, any push to `origin main`, any `uv publish`, any `pyproject.toml` `version` bump beyond what the spec calls for, and on any sub-agent disagreement (sr-dev says BLOCK, sr-sdet says SHIP, etc.). KDR additions land on isolated commits per autonomy decision 2 so they can be reverted independently of task code. See memory: `feedback_autonomous_mode_boundaries.md`.
