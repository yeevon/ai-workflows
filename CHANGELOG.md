...
### Changed — M13 Task 05: branch split — `main` becomes release branch; builder artefacts pruned (2026-04-22)

Closes M13 exit criterion §9. `main` is now the user-facing release
branch; `design_branch` (pushed to the remote before this commit)
carries the builder/auditor workflow. Single forward commit on `main`;
no force-push, no history rewrite.

**Deleted from `main` (builder-only, preserved on `design_branch`):**

- `design_docs/` — entire tree (architecture of record, roadmap, ADRs,
  per-milestone READMEs, task specs, audit issue files,
  `nice_to_have.md`, analysis notes, pre-pivot archive).
- `CLAUDE.md` — Builder / Auditor mode conventions.
- `.claude/commands/` — slash-command definitions (`implement`,
  `audit`, `clean-implement`).
- `tests/skill/` — depends on `design_docs/phases/milestone_9_skill/
  skill_install.md`. Keeping the suite green would require keeping
  that walk-through on `main`, which violates the builder-only
  boundary. The M9 skill itself (`.claude/skills/`) is user-facing
  and stays.
- `scripts/spikes/` — `claude_code_poc.py` exploratory script.

**Adopted on `main` from `origin/design_branch@9560fe2`** (T01–T04
release-prep deltas):

- `pyproject.toml` — T01 metadata polish (`authors`, `urls`,
  `classifiers`, `keywords`) + wheel `force-include` for `migrations/`
  so `uvx --from ai-workflows aiw …` can apply migrations on first
  run.
- `README.md` — T04 trim (235 → 109 lines), user-facing intro with
  `## Install`, `## Getting started`, `## MCP server`, `## Contributing
  / from source`, `## Development`, `## Next`. Line 95 updated to
  point at the actual `design_branch` GitHub URL (T04 shipped a
  placeholder pending T05).
- `docs/architecture.md` + `docs/writing-a-workflow.md` +
  `docs/writing-a-graph-primitive.md` — T03 user-facing walkthroughs
  (rewritten from pre-pivot placeholders); `docs/writing-a-
  component.md` deleted (renamed → `writing-a-graph-primitive.md`).
- `tests/docs/test_docs_links.py` + `tests/docs/test_readme_shape.py`
  — T03 / T04 hermetic doc shape guards.
- `tests/test_wheel_contents.py` — T02 wheel-contents hermetic test.
- `scripts/release_smoke.sh` — T02 release smoke script.
- `tests/test_scaffolding.py` — T03 one-line lockstep rename
  (`writing-a-component.md` → `writing-a-graph-primitive.md`).
- `CHANGELOG.md` — T01 / T02 / T03 / T04 `[Unreleased]` blocks.

**New on `main`:**

- `.github/CONTRIBUTING.md` — one-paragraph pointer at
  `design_branch` + PR-direction note (`design_branch → main` only).
- `tests/test_main_branch_shape.py` — branch-shape invariant test.
  Three functions: `test_design_docs_absence_on_main` (skips on
  `AIW_BRANCH=design`), `test_design_docs_presence_on_design_branch`
  (skips when not on design_branch), `test_contributing_md_exists_
  everywhere` (unconditional). Inversion via `AIW_BRANCH` env flag so
  a single test file covers both branches.

**ACs satisfied:** AC-1 (builder-only paths absent on main — verified
by `git ls-tree`), AC-2 (T01–T04 user-facing slice adopted — every
file listed in the spec's §Deliverables 1 table is present), AC-3
(CONTRIBUTING.md present), AC-4 (`tests/test_main_branch_shape.py`
with three functions), AC-5 (README:95 carries actual URL + builder-
only marker), AC-6 (gates green on main, test count delta audited),
AC-7 (`lint-imports` 4/4 kept), AC-8 (`ruff` clean), AC-10 (branch-
shape test green on both branches via env-flag inversion), AC-11
(this CHANGELOG block), AC-14 (no `ai_workflows/` change).

**Not touched on `main`:** `ai_workflows/` (runtime byte-identical to
pre-split), `migrations/`, `evals/`, `.claude/skills/` (user-facing
M9 skill), `.github/workflows/`, `LICENSE`, `uv.lock`.

**Files NOT on `main` after T05 (live on `design_branch` only):**
task specs (`task_0{1..5}_*.md`), audit issue files, release runbook,
`CLAUDE.md`, slash-command definitions, `nice_to_have.md`, ADRs,
per-milestone READMEs, analysis notes, pre-pivot archive, and
`tests/skill/`.
