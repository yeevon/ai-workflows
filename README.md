...
| M13 — v0.1.0 release + PyPI packaging | Planned (M11 + M14 complete; unblocked) |
...
the same dict without a schema break. T02 close-out live-smoke re-run
of the M9 T04 scenario (fresh Claude Code session → skill →
`run_workflow(planner, goal='Write a release checklist…')` → gate
pause with non-null `plan` + `gate_prompt` → approve → completed)
**passed** against commit baseline `9d03f8d` (MCP run id
`01KPV309SAX702CR8XER1S4WM5`); details in the
[M11 T01 deep analysis](design_docs/phases/milestone_11_gate_review/deep_analysis.md)
and the [M11 README Outcome section](design_docs/phases/milestone_11_gate_review/README.md).
M12 (cascade) and M13 (v0.1.0 release) are now unblocked by this
milestone.

**M14 closed 2026-04-22** (T01 implementation + T02 close-out). Ships
`aiw-mcp --transport http --port <N> --cors-origin <origin>` as a Typer
flag alongside the existing stdio default — same four MCP tools, same
pydantic contracts, same gate-pause projection from M11, now also
reachable over streamable-HTTP for browser-origin consumers (Astro /
React / Vue on the same host). Loopback bind default; no auth at M14
(trigger-gated for a later milestone if a second process on the host
is ever hostile). CORS middleware wires through
`FastMCP.run(transport="http", middleware=[Middleware(CORSMiddleware, …)])`
— not `server.add_middleware(...)` as the task spec initially
prescribed; the FastMCP-3.2.4 API divergence is recorded in
[ADR-0005](design_docs/adr/0005_fastmcp_http_middleware_accessor.md).
Zero schema change (KDR-008 preserved trivially — transport is
additive), zero workflow/graph/primitives diff, zero new layer
contract. Ten new hermetic tests land under
[`tests/mcp/test_http_transport.py`](tests/mcp/test_http_transport.py)
(five at T01 covering the CORS preflight / stdio-unchanged /
round-trip paths; five at T02 absorbing the deep-analysis
regression-guard gaps — stdio-default invariant, HTTP/stdio
schema parity for `run_workflow`, and round-trip envelopes for
`list_runs` / `cancel_run` / `resume_run`). Full suite moves to
612 passed / 5 skipped, 4 contracts kept, ruff clean. Stdio remains
the primary skill surface per KDR-002; every existing Claude Code /
Cursor / Zed registration of `aiw-mcp` keeps working unchanged.
M13 (v0.1.0 release) is now fully unblocked — both its M11 and M14
prerequisites are complete.

## What runs today (post-M14)

- **`aiw-mcp` MCP server** (FastMCP) — four tools exposed per [architecture.md §4.4](design_docs/architecture.md): `run_workflow`, `resume_run`, `list_runs`, `cancel_run`. Schema-first pydantic I/O (auto-derived by FastMCP per KDR-008). CLI and MCP route through one shared dispatch helper [`ai_workflows/workflows/_dispatch.py`](ai_workflows/workflows/_dispatch.py) so both surfaces stay in lockstep. **Stdio is the default transport** — register with Claude Code via `claude mcp add ai-workflows --scope user -- uv run aiw-mcp`, full walkthrough in [design_docs/phases/milestone_4_mcp/mcp_setup.md](design_docs/phases/milestone_4_mcp/mcp_setup.md). **Streamable-HTTP transport** (M14) is opt-in behind `aiw-mcp --transport http --port <N> --cors-origin <origin>` — serves the same schema for browser-origin consumers (Astro / React / Vue); loopback bind default; no auth at M14; FastMCP middleware accessor recorded in [ADR-0005](design_docs/adr/0005_fastmcp_http_middleware_accessor.md); walkthrough in [design_docs/phases/milestone_9_skill/skill_install.md §5](design_docs/phases/milestone_9_skill/skill_install.md). `cancel_run` performs both the Storage-level status flip and (per M6 T02) looks the active run up in a process-local `_ACTIVE_RUNS: dict[run_id, asyncio.Task]` registry and calls `task.cancel()` — satisfies [architecture.md §8.7](design_docs/architecture.md) for the in-flight-abort case that the parallel slice_refactor fan-out made a UX requirement.
...
- **MCP surface** — [ai_workflows/mcp/](ai_workflows/mcp/): `build_server()` factory, pydantic I/O models in `schemas.py`, dual-transport entry point in `__main__.py` (`--transport stdio` default / `--transport http` M14 opt-in), the four tool bodies in `server.py`.
...
Post-M14 snapshot: 612 passed, 5 skipped (the four e2e smokes — `test_planner_smoke.py`, `test_tier_override_smoke.py`, `test_slice_refactor_smoke.py`, `test_ollama_outage_smoke.py` — gated by `AIW_E2E=1`, plus the live-mode eval replay suite gated by `AIW_EVAL_LIVE=1 + AIW_E2E=1`), 4 contracts kept, ruff clean. The +10 tests vs post-M11 come from M14's HTTP-transport suite at [`tests/mcp/test_http_transport.py`](tests/mcp/test_http_transport.py) — five at T01 covering CORS preflight / stdio-unchanged / HTTP round-trip, five at T02 absorbing deep-analysis regression-guard gaps (stdio-default invariant, HTTP/stdio schema parity for `run_workflow`, and round-trip envelopes for `list_runs` / `cancel_run` / `resume_run`).
...
Three milestones remain post-M14. M10 is independent; M12 is unblocked (depended on M11); M13 is the first distributable release and is now unblocked — both prerequisites (M11 gate-pause projection, M14 HTTP transport) are complete.
...
- **M12 — [Tiered audit cascade](design_docs/phases/milestone_12_audit_cascade/README.md)** (planned; unblocked by M11). Ships KDR-011 + [ADR-0004](design_docs/adr/0004_tiered_audit_cascade.md): new `AuditCascadeNode` graph primitive + `auditor-sonnet`/`auditor-opus` `TierConfigs` + per-workflow `audit_cascade_enabled` opt-in + role-tagged `TokenUsage` telemetry + a `run_audit_cascade` MCP tool for standalone artefact audit. Auditor tiers route via the existing `ClaudeCodeSubprocess` over the OAuth CLI (`--model sonnet` / `--model opus`) — KDR-003 preserved, zero `anthropic` SDK surface. The cascade's HumanGate escalation path is reviewable now that the M11 gate-context projection has landed.
- **M13 — [v0.1.0 release + PyPI packaging](design_docs/phases/milestone_13_v0_release/README.md)** (planned; M11 + M14 complete, unblocked). The first distributable release — `uvx --from ai-workflows aiw run planner …` from a clean machine. Polishes `pyproject.toml` metadata, fixes the hatchling wheel-contents bug (`migrations/` currently omitted from the wheel — breaks first-run yoyo on any `uvx`-style install), populates the three empty [docs/](docs/) placeholders against the post-pivot architecture, trims this README from a builder-facing narrative down to a user-facing intro, adds install sections to both the README and the skill doc (with `uvx`-based MCP option), and publishes `0.1.0` to PyPI via a manual `uv publish`. Introduces a **two-branch model** — `design` preserves the full builder workflow (design_docs/, CLAUDE.md, audit files); `main` is release-branch only. M10 / M12 are explicitly *not* prerequisites — they consolidate into a later 0.2.x release. Packaging-only; no runtime feature, no new KDR.
