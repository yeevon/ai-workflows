...
- **M13 — [v0.1.0 release + PyPI packaging](design_docs/phases/milestone_13_v0_release/README.md)** (planned). The first distributable release — `uvx --from ai-workflows aiw run planner …` from a clean machine. Polishes `pyproject.toml` metadata, fixes the hatchling wheel-contents bug (`migrations/` currently omitted from the wheel — breaks first-run yoyo on any `uvx`-style install), adds an install section to this README + a `uvx`-based MCP-server install option to the skill doc, and publishes `0.1.0` to PyPI via a manual `uv publish`. Depends on M11 (publishing before the gate-review projection lands would ship a broken first-impression skill UX). M10 / M12 are explicitly *not* prerequisites — they consolidate into a later 0.2.x release. Packaging-only; no runtime feature, no new KDR.
