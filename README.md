...
| M14 — MCP HTTP transport | Planned (see roadmap) |
| M13 — v0.1.0 release + PyPI packaging | Planned (depends on M11 + M14) |
...
Four milestones are planned post-M11. M10 is independent; M12 now unblocked (depended on M11); M14 enables browser-origin MCP consumption; M13 is the first distributable release (depends on M11 + M14).
...
- **M14 — [MCP HTTP transport](design_docs/phases/milestone_14_mcp_http/README.md)** (planned). `aiw-mcp --transport http` alongside the existing stdio default. Enables browser-origin consumers (Astro / React / Vue) to invoke the MCP surface without spawning a subprocess — drives the interactive CS-300 notes site's question-generation integration. Loopback bind default, optional permissive CORS for configured origins, no auth at M14 (trigger-gated for later). No schema change — FastMCP serialises the same pydantic models over either transport; the M11 gate-pause projection rides through unchanged. Precondition for M13 so the first published wheel covers both transports. No new KDR.
- **M13 — [v0.1.0 release + PyPI packaging](design_docs/phases/milestone_13_v0_release/README.md)** (planned; depends on M11 + M14). The first distributable release — `uvx --from ai-workflows aiw run planner …` from a clean machine. Polishes `pyproject.toml` metadata, fixes the hatchling wheel-contents bug (`migrations/` currently omitted from the wheel — breaks first-run yoyo on any `uvx`-style install), populates the three empty [docs/](docs/) placeholders against the post-pivot architecture, trims this README from a builder-facing narrative down to a user-facing intro, adds install sections to both the README and the skill doc (with `uvx`-based MCP option), and publishes `0.1.0` to PyPI via a manual `uv publish`. Introduces a **two-branch model** — `design` preserves the full builder workflow (design_docs/, CLAUDE.md, audit files); `main` is release-branch only. M10 / M12 are explicitly *not* prerequisites — they consolidate into a later 0.2.x release. Packaging-only; no runtime feature, no new KDR.
